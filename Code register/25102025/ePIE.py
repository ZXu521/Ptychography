...
def make_gaussian_probe(patch, sigma_px, device="cpu", energy_norm=True):
    ...
    energy_norm: 若为 True，保证 sum |P|^2 = 1
    """
...
# Set Gaussian width (pixels). FWHM ≈ 2.355*sigma, take FWHM ≈ 0.5*patch for stability
...
for it in range(1, num_iters + 1):
    ...
    for k, idx in enumerate(order):
        ...
        # —— Object update： max(|P|^2) as scalar denominator ——
        denom_o = torch.max(torch.abs(Pg)**2)+ alpha_o   # Scalar
        Og_new  = Og + beta_o * torch.conj(Pg) * (psi_c - psi_g) / denom_o
        # —— Probe update： max(|Og|^2) as scalar denominator ——
        denom_p = torch.max(torch.abs(Og)**2) + alpha_p  # Scalar
        Pg_new  = Pg + beta_p * torch.conj(Og) * (psi_c - psi_g) / denom_p

        # Write back object patch
        obj_guess[x0:x0+patch, y0:y0+patch] = Og_new
        # Update global probe (single probe case: direct replacement; multi-probe can do sliding average)
        probe_curr = Pg_new
